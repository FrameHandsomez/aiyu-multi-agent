...
async function run(options = {}) {
    ...
    switch (evt.type) {
      case "thinking": {
        ...
        if (preview.length > 10) {
          console.log(H.style.dim(`  ${H.getKawaiiSpinnerFrame(evt.step)} ${preview}${evt.content.length > 120 ? "..." : ""}`));
        }
      ...
      // Legacy fallback — remove once all emitters migrated
      case "tool_call": {
  ...
  (Saved as default: ${currentTrustLevel})\n`);
    ...
    const onToken = (token) => {
      ...
      // Strip markdown asterisks from streamed output (full render in renderResponseBox)
      const cleaned = token.replace(/\*\*([^*]+)\*\*/g, "$1").replace(/(?<!\*)\*([^*]+)\*(?!\*)/g, "$1");
    ...
    try {
      ...
      if (entry.steps?.length > 0) {
        ...
        // Hide token count when 0 (mock provider doesn't count)
        const tokenStr = totalTokens > 0 ? `, ${H.formatTokens(totalTokens)} tokens` : "";
        console.log(H.style.dim(`  [Done · ${(totalMs / 1000).toFixed(1)}s${tokenStr}]`));
      }

      if (streamedOutput) {
        writeStreamBoxBottom();
        // Agent signature footer (content-first, metadata-second)
        console.log(H.renderAgentSignature(agentName));
        console.log("");
      } else {
        console.log("");
