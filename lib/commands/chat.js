  ...
  _init() {
    ...
    const onKeypress = (str, key) => {
      if (key && key.ctrl && !key.meta) {
        ...
        else if (key.name === "v" && this._onPasteImage) this._onPasteImage();
        return;
      }
      ...
      this._clearGhostHint();
      if (!key || key.name === "return" || key.name === "escape") return;
...
function pasteImageFromClipboard() {
  ...
  try {
    if (process.platform === "win32") {
      ...
      execSync(`powershell -Command "Add-Type -AssemblyName System.Windows.Forms; [System.Windows.Forms.Clipboard]::GetImage().Save('${imgPath.replace(/'/g, "''")}')"`, { stdio: "pipe", timeout: 3000 });
...
async function run(options = {}) {
  ...
  while (true) {
    ...
    if (pendingCommand !== null) {
      ...
      getReader()._onPasteImage = () => {
        if (getReader()._resolver) {
          const imgPath = pasteImageFromClipboard();
          if (imgPath) {
            const resolve = getReader()._resolver;
            getReader()._resolver = null;
            resolve(`[image:${imgPath}]`);
          }
        }
      };
