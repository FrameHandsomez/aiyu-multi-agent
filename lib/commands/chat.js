...
class InputReader {
  ...
  _init() {
    ...
    // Ctrl+L → clear screen (readline has no built-in binding for this)
    readline.emitKeypressEvents(process.stdin);
    const onKeypress = (str, key) => {
      if (key && key.ctrl && key.name === "l" && !key.meta) {
        this.clearScreen();
      }
    };
...
// (pasteImageFromClipboard removed — not wired to any keybinding)
