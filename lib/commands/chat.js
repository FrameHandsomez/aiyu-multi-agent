...
const SLASH_COMMANDS = [
  ...
  { cmd: "/trust",       desc: "Cycle trust level: ask → auto → yolo" },
  { cmd: "/yolo",        desc: "Alias for /trust yolo" },
  { cmd: "/allowlist",   desc: "List / revoke command approvals" },
  { cmd: "/agent",      desc: "Switch to a different agent (auto-route or specify name)" },
  { cmd: "/agents",     desc: "List all available agents and their keywords" },
...
async function pickSession() {
  ...
  return new Promise((resolve) => {
    let sel = 0;
    let filterText = "";
    let _drawnLines = 0; // track actual lines written by draw()
    ...
    const cleanup = (selected) => {
      ...
      if (_drawnLines > 0) {
        readline.moveCursor(process.stdout, 0, -_drawnLines);
        readline.cursorTo(process.stdout, 0);
        readline.clearScreenDown(process.stdout);
      }
      resolve(selected);
    ...
    const onKey = (str, key) => {
      if (!key) {
        ...
        if (str && str.length === 1 && str.charCodeAt(0) >= 32) {
          filterText += str;
          sel = 0;
          if (_drawnLines > 0) readline.moveCursor(process.stdout, 0, -_drawnLines);
          draw();
        }
        return;
      }
...
class InputReader {
  ...
  _clearGhostHint() {
    ...
    readline.moveCursor(process.stdout, 0, -1);
    // Reposition cursor to where readline expects it (prompt + input end)
    // so readline's next _refreshLine() won't calculate relative moves from the wrong spot.
    readline.cursorTo(process.stdout, 0);
    const promptLen = stripAnsiSafe(this._rl?._prompt || "").length;
    const cursorPos = (this._rl?.cursor ?? 0);
    readline.moveCursor(process.stdout, promptLen + cursorPos, 0);
    this._ghostHintLine = "";
  }
  ...
  _init() {
    ...
    const onKeypress = (str, key) => {
      ...
      this._ghostTimer = setTimeout(() => {
        ...
        let hint;
        if (line === "/") {
          hint = H.style.dim(`${hits[0].cmd}  — ${hits[0].desc} (+${hits.length - 1} more)`);
        } else if (hits.length === 1) {
          hint = H.style.dim(hits[0].cmd + "  — " + hits[0].desc);
        } else {
          return; // ambiguous partial — don't hint
        }
        this._ghostHintLine = hint;
        // Write hint on the line below, then move cursor back up
        process.stdout.write("\n" + hint);
        readline.moveCursor(process.stdout, 0, -1);
        // After moving up from the hint line the terminal cursor is at the
        // hint's end column, not at the input's end.  We must realign to the
        // exact spot readline expects (prompt length + cursor position) so
        // that its next _refreshLine() computes relative movements correctly.
        readline.cursorTo(process.stdout, 0);
        const promptLen = stripAnsiSafe(this._rl?._prompt || "").length;
        const cursorPos = (this._rl?.cursor ?? 0);
        readline.moveCursor(process.stdout, promptLen + cursorPos, 0);
...
async function pickCommand() {
  ...
  return new Promise((resolve) => {
    ...
    const onKey = (str, key) => {
      if (!key) return;
      if (key.name === 'up')                 { sel = (sel - 1 + N) % N; if (_drawnLines > 0) readline.moveCursor(process.stdout, 0, -_drawnLines); draw(); }
      else if (key.name === 'down')          { sel = (sel + 1) % N;     if (_drawnLines > 0) readline.moveCursor(process.stdout, 0, -_drawnLines); draw(); }
...
async function run(options = {}) {
  ...
  if (!agentName) {
    // Auto-route: pick the best agent based on project context
    const defaultAgent = utils.findDefaultAgent(projectDir);
  ...
  // ── Approval mode from --trust flag ──
  // ask = approve ALL commands (strict), auto = safe=auto+dangerous=ask (default), yolo = no approvals
  // Priority: --trust CLI flag > user config default > "auto"
  const defaultTrust = userConfig.load().default_trust_level || "auto";
  currentTrustLevel = (options.trust || defaultTrust).toLowerCase();
  yoloMode = currentTrustLevel === "yolo";
  strictMode = currentTrustLevel === "ask";
  ...
  if (!H.isCompactMode()) {
    ...
    const trustIcons = { ask: "🔒", auto: "⚖️", yolo: "🔥" };
    console.log(H.style.dim(`    Trust:     ${trustIcons[currentTrustLevel] || ""} ${currentTrustLevel}`));
  ...
  let turnCounter = 0;
  let streamingBoxOpen = false;
  let lastTurnMessageStart = 0; // tracks message count before each send() for /undo

  const stripLeadingAgentProtocol = (text) => {
    if (!text) return "";
    ...
    while (i < lines.length) {
      ...
      if (
        ...
        /Active Agent:/i.test(normalized) ||
        /Skills:/i.test(normalized) ||
        /Rules:/i.test(normalized) ||
        /Sub-?agents:/i.test(normalized) ||
        normalized === "---"
      ) {
        i++;
        continue;
  ...
  const stripMarkdownForStream = (text) => {
    if (!text) return "";
  ...
  const onStep = (evt) => {
    ...
    switch (evt.type) {
      case "thinking": {
        if (streamingBoxOpen) {
          break; // streaming box handles the display; don't overwrite with status bar
        }
        // Only show thinking line if there are tool calls coming (multi-step)
        // For single-step (final answer), skip — the streamed output is enough
        // Preview is shown only if content is substantial
        const cleaned = stripMarkdownForStream(stripLeadingAgentProtocol(evt.content || ""));
        const preview = cleaned.replace(/\n/g, " ").slice(0, 120).trim();
        if (preview.length > 10) {
          console.log(H.style.dim(`  ${H.getKawaiiSpinnerFrame(evt.step)} ${preview}${cleaned.length > 120 ? "..." : ""}`));
        }
      ...
      case "tool_started": {
        if (streamingBoxOpen) {
          ...
          streamRawBuffer = "";
          streamPrintedCodepoints = 0;
        }
        console.log(H.renderToolPreparing(evt.tool));
        startStatusBar(agentName, model, provider);
        break;
      }
      case "tool_completed": {
        console.log(H.renderToolCompleted(evt.tool, evt.args, evt.duration_ms));
        if (evt.error) {
          console.log(H.style.statusBad(`  ✗ ${evt.tool}`) + H.style.error(`: ${evt.error}`));
        } else if ((evt.tool === "write" || evt.tool === "edit") && evt.prevContent && evt.newContent) {
  ...
  while (true) {
    ...
    if (expandedLower === "/agents") {
      const agents = agentRouter.listAgents(projectDir);
      if (agents.length === 0) {
        console.log(H.style.dim("\n  No agents found.\n"));
      } else {
        console.log(H.style.accent(`\n  Available Agents (${agents.length}):\n`));
        for (const a of agents) {
          const kw = a.keywords.slice(0, 6).join(", ");
          console.log(H.style.text(`    ${H.style.accent(a.name.padEnd(28))} ${H.style.dim(kw)}`));
        }
        console.log("");
      }
      continue;
    ...
    if (expandedLower === "/agent" || expandedLower.startsWith("/agent ")) {
      const targetName = message.slice("/agent".length).trim();
      if (!targetName) {
        // No name given — auto-route based on last user message
        if (lastUserMessage) {
          const routed = agentRouter.route(lastUserMessage, projectDir, { preferAgent: agentName });
          if (routed) {
            console.log(H.style.accent(`\n  Auto-routed → ${H.style.accent(routed.name)} (score: ${routed.score}, method: ${routed.method})\n`));
            agentName = routed.name;
          } else {
            console.log(H.style.dim("\n  No matching agent found for your last message.\n"));
          }
        } else {
          console.log(H.style.dim("\n  Usage: /agent <name>  or  /agent  (auto-route from last message)\n"));
        }
      } else {
        // Validate agent exists
        const agents = agentRouter.listAgents(projectDir);
        const found = agents.find(a => a.name === targetName || a.name === targetName.replace(/\s+/g, "-"));
        if (found) {
          agentName = found.name;
          console.log(H.style.statusGood(`\n  Switched to agent: ${H.style.accent(agentName)}\n`));
        } else {
          // Try auto-routing with the targetName as input
          const routed = agentRouter.route(targetName, projectDir);
          if (routed) {
            agentName = routed.name;
            console.log(H.style.accent(`\n  Auto-routed → ${H.style.accent(agentName)} (score: ${routed.score}, method: ${routed.method})\n`));
          } else {
            console.log(H.style.error(`\n  Agent not found: "${targetName}". Type /agents to see available agents.\n`));
          }
        }
      }
      // Re-create session with new agent
      try {
        const prevMessages = session ? session.getMessages() : [];
        session = agentRuntime.createChatSession({ agentName, projectDir, provider, model, onStep, strictMode, yoloMode });
        if (prevMessages.length > 0) session.setMessages(prevMessages);
        saveSession(sessionId, agentName, provider, model, sessionTitle, session.getMessages());
      } catch (err) {
        console.log(H.style.error(`  Error switching agent: ${err.message}\n`));
      }
      continue;
    }
    ...
    if (expandedLower === "/new" || expandedLower === "/clear" || expandedLower === "clear") {
      try {
        session = agentRuntime.createChatSession({ agentName, projectDir, provider, model, strictMode, yoloMode });
        lastUserMessage = "";
        pendingImages.length = 0;
        pendingFiles.length = 0;
        imageCounter = 0;
    ...
    if (expandedLower === "/undo") {
      ...
      if (hist.length === 0 || lastTurnMessageStart === 0) {
        console.log(H.style.statusWarn("\n  Nothing to undo in this session.\n"));
      } else {
        const msgs = session.getMessages();
        const removed = msgs.length - lastTurnMessageStart;
        msgs.splice(lastTurnMessageStart);
        hist.pop();
        lastTurnMessageStart = 0;
        console.log(H.style.accent(`\n  Undid last turn (${removed} message(s) removed).\n`));
      }
      continue;
    }
    ...
    // /trust — cycle trust level: ask → auto → yolo
    // /yolo — shortcut for /trust yolo
    if (expandedLower === "/trust" || expandedLower === "/yolo" || message.startsWith("/trust ")) {
      const TRUST_LEVELS = ["ask", "auto", "yolo"];
      const TRUST_LABELS = {
        ask:  "🔒 ask  — approve ALL commands before running",
        auto: "⚖️ auto — safe commands auto-approved, dangerous asks",
        yolo: "🔥 yolo — no approvals, everything runs",
      };
      const sub = message.startsWith("/trust ") ? message.slice("/trust ".length).trim().toLowerCase() : "";
      if (expandedLower === "/yolo") {
        currentTrustLevel = "yolo";
      } else if (sub && TRUST_LEVELS.includes(sub)) {
        currentTrustLevel = sub;
      } else {
        // Cycle: ask → auto → yolo → ask
        const idx = TRUST_LEVELS.indexOf(currentTrustLevel);
        currentTrustLevel = TRUST_LEVELS[(idx + 1) % TRUST_LEVELS.length];
      }
      yoloMode = currentTrustLevel === "yolo";
      strictMode = currentTrustLevel === "ask";
      // Save as new default in user config
      userConfig.setTrustLevel(currentTrustLevel);
      console.log(`\n  ${TRUST_LABELS[currentTrustLevel]}
  (Saved as default: ${currentTrustLevel})\n`);
      continue;
    }

    // /allowlist — list or revoke command approvals
    if (expandedLower === "/allowlist" || expandedLower.startsWith("/allowlist ")) {
      const approvalStore = require("../core/approval-store");
      const sub = message.slice("/allowlist".length).trim();
      if (sub.startsWith("revoke")) {
        const target = sub.slice("revoke".length).trim();
        if (!target) {
          console.log(H.style.statusWarn("\n  Usage: /allowlist revoke <command-or-hash>\n"));
          continue;
        }
        const removed = approvalStore.revokeApproval(target);
        console.log(removed
          ? H.style.statusGood(`\n  ✅ Revoked approval for: ${target}\n`)
          : H.style.statusWarn(`\n  ⚠ No approval found for: ${target}\n`));
        continue;
      }
      // Default: list approvals
      const { session, persistent } = approvalStore.listApprovals();
      console.log(H.style.accent("\n  ╭─ Command Approvals ─" + "─".repeat(38) + "╮"));
      if (session.length > 0) {
        console.log(H.style.dim("  │ Session:"));
        for (const a of session) {
          console.log(H.style.dim(`  │   ${a.command}`));
        }
      }
      if (persistent.length > 0) {
        console.log(H.style.dim("  │ Persistent:"));
        for (const a of persistent) {
          console.log(H.style.dim(`  │   ${a.scope.padEnd(8)} ${a.command}`));
        }
      }
      if (session.length === 0 && persistent.length === 0) {
        console.log(H.style.dim("  │ (empty)"));
      }
      console.log(H.style.accent("  ╰" + "─".repeat(62) + "╯\n"));
      continue;
    }
    ...
    // Auto-route suggestion: check if another agent is a better fit
    if (msgToSend && !msgToSend.startsWith("/")) {
      const routed = agentRouter.route(msgToSend, projectDir, { preferAgent: agentName });
      if (routed && routed.name !== agentName && routed.score >= 10) {
        console.log(H.style.dim(`  💡 Tip: ${H.style.accent(routed.name)} may be a better fit (score: ${routed.score}). Type /agent to switch.`));
      }
    }
    ...
    // (stripLeadingAgentProtocol/stripMarkdownForStream are defined at session scope)
    ...
    const writeStreamBoxBottom = () => {
      ...
      streamingBoxOpen = false;
    };

    const onToken = (token) => {
      streamRawBuffer += token;

      const display = stripMarkdownForStream(stripLeadingAgentProtocol(streamRawBuffer));
      const displayChars = [...display];
      const newChars = displayChars.slice(streamPrintedCodepoints);
      streamPrintedCodepoints = displayChars.length;

      if (!streamedOutput) {
        const hasVisibleContent = newChars.some((c) => c.replace(/\s/g, "") !== "");
      ...
      for (const ch of newChars) {
        ...
        streamLineVisibleLen += H.visibleLen(ch);
      }
    };

    try {
      lastTurnMessageStart = session.getMessages().length;
      const entry = await session.send(msgToSend, { onToken });
      stopStatusBar();

      const finalContent = stripLeadingAgentProtocol(entry.content).replace(/\uFFFD+/g, "");

      // Build footer: Done · Xs · agent-name (kaomoji)
      const totalMs = entry.steps?.length > 0
        ? entry.steps.reduce((s, st) => s + (st.duration_ms || 0), 0)
        : 0;
      const totalTokens = entry.usage?.totalTokens || 0;
      const tokenStr = totalTokens > 0 ? ` · ${H.formatTokens(totalTokens)} tok` : "";
      const kaomoji = H.PERSONA_KAOMOJI[agentName] || H.PERSONA_KAOMOJI["default"];
      const elapsed = totalMs > 0 ? `Done · ${(totalMs / 1000).toFixed(1)}s${tokenStr}` : `Done${tokenStr}`;
      const footer = `  ${elapsed} · ${agentName} ${kaomoji}`;

      if (streamedOutput) {
        writeStreamBoxBottom();
      } else {
        console.log("");
        console.log(H.renderResponseBox(H.PROMPT_SYMBOLS.agentRunning + " Aiyu", finalContent));
      }
      console.log(H.style.dim(footer));
      console.log("");
      ...
      stopStatusBar();
      if (streamingBoxOpen) writeStreamBoxBottom(); // close box if error hit mid-stream
      if (err.cancelled) {
