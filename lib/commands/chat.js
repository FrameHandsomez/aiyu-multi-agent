...
class InputReader {
  ...
  /**
   * Ctrl+V handler — check clipboard for image.
   * If image found: save it, show [🖼 Image #N], clear any text the terminal pasted.
   * If no image: let the terminal's text paste go through normally.
   */
  _handleCtrlV() {
    if (!this._onPasteImage) return; // not in chat loop yet
    const imgPath = pasteImageFromClipboard();
    if (imgPath) {
      // Image found — clear any text the terminal may have pasted into readline
      if (this._rl) {
        // Reset the readline line to empty
        this._rl.line = "";
        this._rl.cursor = 0;
        this._rl._refreshLine();
      }
      this._onPasteImage(imgPath);
    }
    // If no image in clipboard, do nothing — terminal's text paste goes through
  }
  ...
  _init() {
    ...
    const onKeypress = (str, key) => {
      if (key && key.ctrl && !key.meta) {
        ...
        else if (key.name === "v") this._handleCtrlV();
        return;
      }
 ...
 *
 * On Windows: writes a .ps1 script to avoid $ escaping issues with
 * inline PowerShell commands through cmd.exe / Node.js execSync.
 */
function pasteImageFromClipboard() {
  ...
  try {
    ...
    if (process.platform === "win32") {
      // Write a temp .ps1 script — avoids $ escaping hell with inline -Command
      const scriptPath = path.join(tmpDir, `_clip-${Date.now()}.ps1`);
      const script = [
        "Add-Type -AssemblyName System.Windows.Forms",
        `$img = [System.Windows.Forms.Clipboard]::GetImage()`,
        `if ($img -ne $null) {`,
        `  $img.Save('${imgPath.replace(/'/g, "''")}')`,
        `  Write-Host 'OK'`,
        `} else {`,
        `  Write-Host 'NO_IMAGE'`,
        `}`,
      ...
      if (result === "OK" && fs.existsSync(imgPath) && fs.statSync(imgPath).size > 0) {
        return imgPath;
      }
...
async function run(options = {}) {
  ...
  while (true) {
    ...
    if (pendingCommand !== null) {
      ...
      // Ctrl+V while idle → paste image from clipboard
      getReader()._onPasteImage = (imgPath) => {
        imageCount++;
        lastImagePath = imgPath;
        console.log(H.style.dim(`\n  [🖼 Image #${imageCount}]`));
        // Re-prompt for next input (user can type message or just Enter)
        if (getReader()._rl) {
          getReader()._rl.setPrompt(H.style.accent(H.PROMPT_SYMBOLS.idle) + " ");
          getReader()._rl.prevRows = 0;
          getReader()._rl.prompt();
        }
      };
      message = await readInput(H.PROMPT_SYMBOLS.idle);
      getReader()._onRetry = null;
      getReader()._onPasteImage = null;
    }

    if (!message || message === "exit" || message === "quit") {
      // If user pressed Enter with an image pending, send image-only message
      if (!message && lastImagePath) {
        message = "";
    ...
    if (expandedLower === "history" || expandedLower === "/history") {
      ...
      history.forEach((entry, i) => {
        console.log(H.style.dim(`  [${i + 1}] ${entry.role}: ${entry.content.slice(0, 100)}${entry.content.length > 100 ? "..." : ""}`));
        if (entry.toolCalls?.length > 0) {
          entry.toolCalls.forEach(tc => {
            const icon = tc.error ? H.style.statusBad("✗") : H.style.statusGood("✓");
            console.log(`    ${icon} ${tc.tool}`);
          });
        }
      });
      console.log("");
    ...
    if (lastImagePath && !msgToSend.startsWith("/")) {
      msgToSend = `[image:${lastImagePath}]\n${msgToSend}`;
      lastImagePath = "";
    }
    ...
    // Track if user pressed Ctrl+C during AI processing
    ...
    try {
      ...
      if (wasCancelled) {
        console.log(H.style.statusWarn("  [Response discarded — cancelled]\n"));
        continue;
      }
