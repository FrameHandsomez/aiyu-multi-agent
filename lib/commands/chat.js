...
const SLASH_COMMANDS = [
  ...
  { cmd: "/yolo",        desc: "Toggle yolo mode (skip all approvals)" },
  { cmd: "/trust",       desc: "Trust current project (session approvals)" },
...
async function run(options = {}) {
  ...
  while (true) {
    ...
    if (expandedLower === "/yolo") {
      yoloMode = !yoloMode;
      console.log(yoloMode
        ? H.style.statusWarn("\n  🔥 YOLO mode ON — all command approvals skipped\n")
        : H.style.statusGood("\n  ✅ YOLO mode OFF — approval checks restored\n"));
      continue;
    }

    // /trust — trust current project (session-approve all moderate commands)
    if (expandedLower === "/trust") {
      const approvalStore = require("../core/approval-store");
      const { classify } = require("../core/command-classifier");
      const { ALLOWED_COMMANDS } = require("../core/guardrails");
      // Pre-approve all moderate-tier commands for this session
      const moderateCmds = ALLOWED_COMMANDS.filter(cmd => {
        const { tier } = classify(cmd);
        return tier === "moderate";
      });
      for (const cmd of moderateCmds) {
        approvalStore.recordApproval(cmd, "session", projectDir);
      }
      console.log(H.style.statusGood(`\n  ✅ Trusted project — ${moderateCmds.length} moderate commands session-approved\n`));
      continue;
    }
