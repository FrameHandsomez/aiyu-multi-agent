...
async function run(projectDir, options = {}) {
  if (config.configExists(projectDir)) {
    console.log(H.style.statusWarn(".agent/ or .windsurf/ already exists. Run `aiyu-multi-agent update` instead.\n"));
    return;
  }

  if (options.dryRun) {
    console.log(H.style.accent("[DRY RUN] Would run init...\n"));
    return;
  }
  ...
  if (options.interactive) {
    ...
    const interactive = await inquirer.prompt([
      {
        ...
        choices: USE_CASES.map((uc) => ({
          name: `${uc.name}  — ${H.style.dim(uc.description)}`,
          short: uc.name,
          value: uc.value,
        })),
      },
    ...
    console.log(H.style.accent(`\n⚔︎ Aiyu MultiAgent — Quick Init`));
    console.log(H.style.dim(`   Provider: ${answers.provider} | Memory: ${answers.memory} | Guardrails: on\n`));
  }
  ...
  try {
    ...
    spinner.succeed(H.style.statusGood("Agent config generated!"));

    if (options.windsurfOnly) {
      console.log(`\n  ${H.style.dim("Created:")} ${H.style.text(".windsurf/agents/" + answers.agentName + ".md")}`);
      console.log(`  ${H.style.dim("Created:")} ${H.style.text(".windsurf/skills/core/ (built-in skills)")}`);
      console.log(`  ${H.style.dim("Created:")} ${H.style.text(".windsurf/config.yaml")}`);
      console.log(`  ${H.style.dim("Created:")} ${H.style.text(".windsurf/tests/" + answers.agentName + ".test.md")}`);
      console.log(`  ${H.style.dim("Created:")} ${H.style.text(".windsurfrules")}`);
    } else if (options.agentOnly) {
      console.log(`\n  ${H.style.dim("Created:")} ${H.style.text(".agent/agents/" + answers.agentName + ".md")}`);
      console.log(`  ${H.style.dim("Created:")} ${H.style.text(".agent/skills/core/ (built-in skills)")}`);
      console.log(`  ${H.style.dim("Created:")} ${H.style.text(".agent/config.yaml")}`);
      console.log(`  ${H.style.dim("Created:")} ${H.style.text(".agent/tests/" + answers.agentName + ".test.md")}`);
    } else {
      console.log(`\n  ${H.style.dim("Created:")} ${H.style.text(".agent/agents/" + answers.agentName + ".md")}`);
      console.log(`  ${H.style.dim("Created:")} ${H.style.text(".agent/skills/core/ (built-in skills)")}`);
      console.log(`  ${H.style.dim("Created:")} ${H.style.text(".agent/config.yaml")}`);
      console.log(`  ${H.style.dim("Created:")} ${H.style.text(".agent/tests/" + answers.agentName + ".test.md")}`);
      console.log(`  ${H.style.dim("Linked:")}  ${H.style.text(".windsurf/ → .agent/")}`);
      console.log(`  ${H.style.dim("Created:")} ${H.style.text(".windsurfrules")}`);
    }

    console.log(`\n  ${H.style.dim("Next:")} ${H.style.accent("aiyu-multi-agent add skill <name>")}`);
    console.log(`        ${H.style.accent("aiyu-multi-agent test")}\n`);
  } catch (err) {
    spinner.fail(H.style.error("Init failed"));
