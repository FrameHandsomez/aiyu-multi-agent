...
async function run(options = {}) {
  ...
  if (summary.topCommands.length > 0) {
    ...
    summary.topCommands.forEach(c => {
      console.log(`    ${H.style.text(c.name.padEnd(15))} ${H.style.dim(c.count + "x")}`);
    });
  }
      ...
      try {
        ...
        if (Object.keys(toolStats).length > 0) {
          ...
          sorted.forEach(([tool, count]) => {
            console.log(`    ${H.style.text(tool.padEnd(20))} ${H.style.dim(count + "x")}`);
          });
        }
        ...
        if (latencyCount > 0) {
          ...
          console.log(H.style.accent("\n  Performance:"));
          console.log(`    ${H.style.dim("Avg latency:")}    ${H.style.text(avgLatency + "s")}`);
          console.log(`    ${H.style.dim("Total steps:")}    ${H.style.text(stepCount)}`);
          if (latencyCount > 0) {
            console.log(`    ${H.style.dim("Error rate:")}     ${H.style.statusWarn(((errorCount / latencyCount) * 100).toFixed(1) + "%")}`);
          }
        }
  ...
  if (summary.skillInstalls > 0) {
    console.log(H.style.accent("\n  Skills:"));
    console.log(`    ${H.style.dim("Installed:")} ${H.style.text(summary.skillInstalls)}`);
  }

  // Tests
  if (summary.testRuns > 0) {
    console.log(H.style.accent("\n  Tests:"));
    console.log(`    ${H.style.dim("Runs:")}      ${H.style.text(summary.testRuns)}`);
    console.log(`    ${H.style.dim("Pass rate:")} ${H.style.text(summary.testPassRate)}`);
  }
