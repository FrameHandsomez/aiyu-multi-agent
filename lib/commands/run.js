...
async function run(input, options = {}) {
  ...
  // Auto-route: if no agent specified, pick the best match from user input
  if (!agentName) {
    const routed = agentRouter.route(input, projectDir);
    if (routed) {
      agentName = routed.name;
      if (!jsonMode) {
        console.log(H.style.dim(`  Auto-routed → ${H.style.accent(agentName)} (score: ${routed.score}, method: ${routed.method})`));
      }
    } else {
      agentName = utils.findDefaultAgent(projectDir);
    }
  }
