...
function detect(projectDir) {
  // Return cached profile if already detected (avoids re-logging on /clear, /new)
  if (detectCache.has(projectDir)) return detectCache.get(projectDir);
  ...
  detectCache.set(projectDir, profile);
  return profile;
}
