...
function createChatSession(options) {
  ...
  let _provider = overrideProvider ?? agentSpec.provider ?? "mock";
  let _model = overrideModel ?? agentSpec.model ?? "gpt-4";
  const outputFormat = options.outputFormat ?? agentSpec.outputFormat ?? "text";
  const chatProfile = agentSystem.detect(projectDir);
  const systemPrompt = buildSystemPrompt(agentSpec, skillInstructions, chatProfile, { provider: _provider, model: _model });
  ...
  return {
    ...
    async send(userInput, opts = {}) {
      ...
      const chatLlmOpts = { provider: _provider, model: _model, abortSignal: _abortController.signal };
      ...
      const chatTraceId = tracing.startTrace(`chat.${agentSpec.name}`, { agentName: agentSpec.name, provider: _provider });
      ...
      for (let step = 0; step < maxChatSteps; step++) {
        ...
        for (const tc of toolCalls) {
          ...
          if (tool) {
            ...
            try {
              if (onStep) onStep({ type: "tool_started", step: step + 1, tool: resolvedName, args: tc.args });
              const toolArgs = { ...(tc.args || {}), projectRoot: projectDir, _strictMode: options.strictMode, _yoloMode: options.yoloMode };
              ...
              if (onStep) onStep({ type: "tool_completed", step: step + 1, tool: resolvedName, args: tc.args, duration_ms: toolDuration, result: typeof result === "string" ? result.slice(0, 200) : JSON.stringify(result).slice(0, 200) });
              ...
              if (onStep) onStep({ type: "tool_completed", step: step + 1, tool: resolvedName, args: tc.args, duration_ms: toolDuration, error: err.message });
