...
/**
 * Async sandbox exec with approval flow.
 * Uses command-classifier + approval-prompt for Hermes-style approval.
 *
 * @param {string} cmd       Command or binary name
 * @param {string[]} [args]  Arguments
 * @param {object} [options]
 * @param {string} [options.projectRoot]  For project-scoped allowlist
 * @param {boolean} [options.strictMode]  Ask approval even for safe commands
 * @param {boolean} [options.yoloMode]    Skip all approvals
 * @returns {Promise<string>}  Command output
 */
async function sandboxExecWithApproval(cmd, args = [], options = {}) {
  const { projectRoot, strictMode, yoloMode } = options;
  const base = cmd.includes(path.sep) ? path.basename(cmd) : cmd;

  // Build full command string for classification
  const fullCmd = [base, ...(args || [])].join(" ");

  // If command not in ALLOWED_COMMANDS at all, classifier will mark dangerous
  // and approval-prompt will handle it.

  // Check approval (async — may prompt user)
  const { promptApproval } = require("./approval-prompt");
  const { approved } = await promptApproval(fullCmd, { projectRoot, strictMode, yoloMode });

  if (!approved) {
    const err = new Error(`Command "${base}" denied by user`);
    err.code = "APPROVAL_DENIED";
    throw err;
  }

  // Approved — run via standard sandboxExec
  // For commands not in ALLOWED_COMMANDS but user-approved, use execFileSync directly
  if (!ALLOWED_COMMANDS.includes(base)) {
    const { execFileSync } = require("child_process");
    const env = options.env || { ...process.env };
    if (!options.env) {
  ...
  return sandboxExec(cmd, args, options);
}

module.exports = { pathTraversal, safeWrite, rateLimit, sandboxExec, sandboxExecWithApproval, ALLOWED_COMMANDS, BLOCKED_FLAGS };
