...
// ── Persona-distinct Kaomoji (replaces generic 🤖) ────────────────────
const PERSONA_KAOMOJI = {
  "accessibility-specialist":  "(◕‿◕)♿",
  "frontend-specialist":       "(｡◕‿◕｡)✧",
  "frontend-design":           "(｡◕‿◕｡)✧",
  "backend-specialist":        "╰( ´◓ω◔`)╯",
  "backend-orchestrator":      "╰( ´◓ω◔`)╯",
  "debugging":                 "(¬､¬)🔍",
  "security-auditor":          "⚔(ಠ_ಠ)",
  "security":                  "⚔(ಠ_ಠ)",
  "mobile-developer":          "(◕‿◕)📱",
  "devops":                    "⚙(◕‿◕)",
  "database-architect":        "(◕‿◕)🗄",
  "data-layer":                "(◕‿◕)🗄",
  "quality-gate":              "✓(◕‿◕)",
  "testing":                   "✓(◕‿◕)",
  "documentation":             "📝(◕‿◕)",
...
/**
 * Render agent signature footer (Hermes pattern: content-first, signature as metadata)
 * e.g., "                    ─ accessibility-specialist (◕‿◕)♿"
 */
function renderAgentSignature(agentName) {
  const kaomoji = PERSONA_KAOMOJI[agentName] || PERSONA_KAOMOJI["default"];
  return style.dim(`                    ─ ${agentName} ${kaomoji}`);
}
...
/**
 * Convert basic markdown to chalk-styled text.
 * Handles: **bold**, *italic*, `code`, ```code blocks```, - lists, > quotes
 */
  ...
  for (const line of lines) {
    ...
    if (line.startsWith("```")) {
      ...
      if (inCodeBlock) {
        ...
        result.push(style.dim("  ┌" + (lang ? " " + lang : "") + BOX.horizontal.repeat(40)));
      } else {
        result.push(style.dim("  └" + BOX.horizontal.repeat(42)));
      }
      continue;
    }
    if (inCodeBlock) {
      result.push(style.dim("  │ ") + chalk.hex(COLORS.dimGold)(line));
      continue;
    }

    // Heading
    if (line.startsWith("### ")) { result.push(style.accent("  " + line.slice(4))); continue; }
    if (line.startsWith("## "))  { result.push(style.accent("  " + line.slice(3))); continue; }
    if (line.startsWith("# "))   { result.push(style.accent("  " + line.slice(2))); continue; }

    // Block quote
    if (line.startsWith("> ")) { result.push(style.dim("  │ ") + style.dim(line.slice(2))); continue; }

    // Unordered list
    if (/^(\s*)[-*] /.test(line)) {
      const indent = line.match(/^(\s*)/)[1];
      const content = line.trim().slice(2);
      result.push(indent + style.accent("  • ") + renderInlineMarkdown(content));
      continue;
    }
    ...
    if (olMatch) {
      ...
      const num = olMatch[2];
      const content = line.trim().slice(num.length + 2);
      result.push(indent + style.accent(`  ${num}. `) + renderInlineMarkdown(content));
      continue;
    }
...
/** Convert inline markdown (**bold**, *italic*, `code`) to chalk */
function renderInlineMarkdown(text) {
  // Inline code: `text`
  text = text.replace(/`([^`]+)`/g, (_, c) => chalk.hex(COLORS.dimGold).bgHex(COLORS.darkBg)(" " + c + " "));
  // Bold: **text**
  text = text.replace(/\*\*([^*]+)\*\*/g, (_, c) => chalk.bold(c));
  // Italic: *text* (avoid matching inside bold)
  text = text.replace(/(?<!\*)\*([^*]+)\*(?!\*)/g, (_, c) => chalk.italic(c));
  return text;
}
...
function renderResponseBox(title, body, width) {
  ...
  // Apply markdown-lite rendering to body
  body = renderMarkdownLite(body);
...
/**
 * Format: ⚔︎ model │ tokens │ % │ elapsed │ lastStep
 * e.g.   ⚔︎ claude-opus-4 │ 12K/200K │ 6% │ 2m 30s │ 45.2s
 */
...
function renderStartupBanner({ version, model, contextWindow, width }) {
  ...
  // Render art centered above the title box
  const artLines = AIYU_ASCII.split("\n");
  const artMaxVis = Math.max(...artLines.map(l => visibleLen(l)));

  const centeredArt = artLines.map(line => {
    const vis = visibleLen(line);
    const pad = Math.max(0, w - vis);
    const left = Math.floor(pad / 2);
    return " ".repeat(left) + style.accent(line);
  });
  ...
  const titlePad = Math.max(0, boxW - titleVisLen);
  const subPad = Math.max(0, boxW - subVisLen);

  const top = BANNER_BOX.topLeft + BANNER_BOX.horizontal.repeat(boxW + 2) + BANNER_BOX.topRight;
  ...
  const bot = BANNER_BOX.bottomLeft + BANNER_BOX.horizontal.repeat(boxW + 2) + BANNER_BOX.bottomRight;

  const boxPad = Math.max(0, Math.floor((w - boxW - 4) / 2));
  const boxIndent = " ".repeat(boxPad);
  const boxLines = [top, mid1, mid2, bot].map(l => boxIndent + (l === top || l === bot ? style.border(l) : l));

  return [...centeredArt, "", ...boxLines].join("\n");
}
...
function _resolveToolIcon(toolName) {
  if (TOOL_ICONS[toolName]) return TOOL_ICONS[toolName];
  const parts = toolName.split(".");
  // Namespaced tools → try base verb first, then namespace
  const base = parts.pop();
  if (TOOL_ICONS[base]) return TOOL_ICONS[base];
  const namespace = parts[0];
  if (TOOL_ICONS[namespace]) return TOOL_ICONS[namespace];
  return TOOL_ICONS.default;
}

function renderToolCall({ tool, args, durationMs }) {
  const icon = _resolveToolIcon(tool);
...
// ── Tool Lifecycle — Hermes-style "preparing…" + completed ──────────
...
/** Print before tool runs: "┊ 📚 preparing skill_view…" */
function renderToolPreparing(toolName) {
  const icon = _resolveToolIcon(toolName);
  return "  " + style.dim("\u250A ") + icon + " " + style.dim(`preparing ${toolName}…`);
}

/** Print after tool finishes: "┊ 📚 skill   hermes-agent  0.0s" */
function renderToolCompleted(toolName, args, durationMs) {
  const icon = _resolveToolIcon(toolName);
  const dur = durationMs !== undefined ? (durationMs / 1000).toFixed(1) + "s" : "";
  const prefix = "\u250A ";

  // Per-tool formatters (mirrors Hermes get_cute_tool_message)
  const fmt = (verb, detail) => {
    return "  " + style.dim(prefix) + icon + " " + style.text(verb.padEnd(10)) + " " + style.dim(_trunc(detail, 38)) + "  " + style.dim(dur);
  };
  ...
  switch (toolName) {
    ...
    case "fetch.url": {
      const url = args?.url || "";
      return fmt("fetch", url);
    }
    case "agent.delegate": {
      const agent = args?.agent || "";
      return fmt("delegate", agent);
    }
    case "memory.save": {
      const key = args?.key || "";
      return fmt("mem-save", key);
    }
    case "memory.load": {
      const key = args?.key || "";
      return fmt("mem-load", key);
    }
    case "web.search": {
      const q = args?.query || "";
      return fmt("search", q);
    ...
    case "plan.create": {
      const tasks = Array.isArray(args?.tasks) ? `${args.tasks.length} tasks` : "";
      return fmt("plan", tasks);
    }
    case "plan.update": {
      return fmt("plan", `${args?.planId || ""} → ${args?.status || ""}`);
    }
    case "plan.list": {
      return fmt("plan", "list");
    }
    default: {
      const detail = args ? JSON.stringify(args).slice(0, 42) : "";
      return fmt(toolName, detail);
    }
  }
}
...
let _compactOverride = null; // null = auto, true/false = manual

function isCompactMode() {
  if (_compactOverride !== null) return _compactOverride;
  return (process.stdout.columns || 80) < 40;
}

function setCompactMode(val) {
  _compactOverride = val;
}
