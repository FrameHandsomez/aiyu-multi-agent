...
async function runAgent(options) {
  ...
  for (let step = 0; step < maxSteps; step++) {
    ...
    for (const toolCall of toolCalls) {
      ...
      if (onToolEvent) onToolEvent("tool_started", { tool: resolvedName, args: toolCall.args, step: step + 1 });
      const toolSpanId = tracing.startSpan(traceId, `tool.${resolvedName}`, { tool: resolvedName });
      try {
        const toolArgs = { ...toolCall.args, projectRoot: projectDir, _currentAgent: agentName, _provider: provider, _model: model, _strictMode: options.strictMode, _yoloMode: options.yoloMode };
        ...
        if (onToolEvent) onToolEvent("tool_completed", { tool: resolvedName, args: toolCall.args, duration_ms: toolDuration, result, step: step + 1 });
        ...
        if (onToolEvent) onToolEvent("tool_completed", { tool: resolvedName, args: toolCall.args, duration_ms: toolDuration, error: err.message, step: step + 1 });
    ...
    if (totalChars > MAX_CONTEXT_CHARS) {
      ...
      // If still over limit after trimming, drop oldest non-system messages (but keep pairs)
