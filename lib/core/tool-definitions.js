...
const BUILTIN_TOOLS = {
  ...
  "shell.exec": async (args) => {
    ...
    try {
      ...
      // Block command substitution — dangerous regardless of trust level
      if (/\$\(/.test(command) || /`[^`\n]*`/.test(command)) {
        return { error: `Command blocked: forbidden pattern detected in command`, exitCode: 1 };
      }

      // Block commands not in ALLOWED_COMMANDS before reaching the approval flow
      if (!guardrails.ALLOWED_COMMANDS.includes(base) && !args._yoloMode) {
      ...
      // Use approval flow: classify → check allowlist → prompt if needed → execute
      const result = await guardrails.sandboxExecWithApproval(base, cmdArgs, {
        timeout,
        cwd: safeCwd,
        projectRoot,
        strictMode: !!args._strictMode,
        yoloMode: !!args._yoloMode,
      });
      return { stdout: result.trim(), exitCode: 0 };
    } catch (e) {
      if (e.code === "APPROVAL_DENIED") {
        return { error: `Command denied by user: ${e.message}`, exitCode: 126 };
      }
  ...
  "agent.delegate": async (args) => {
    ...
    try {
      ...
      const provider = args._provider || agentSpec.provider || "mock";
      const model = args._model || agentSpec.model || "gpt-4";
      const systemPrompt = buildSystemPrompt(agentSpec, skillInstructions, projectProfile, { provider, model });
      ...
      const delegateTimeout = new Promise((_, reject) =>
        setTimeout(() => reject(new Error(`Delegate timeout after ${DELEGATE_TIMEOUT_MS / 1000}s`)), DELEGATE_TIMEOUT_MS)
      );
      ...
      const runPromise = (async () => {
        for (let step = 0; step < maxSteps; step++) {
          ...
          try {
            ...
            return { error: `Delegate LLM call failed: ${err.message}` };
          }
          ...
          if (toolCalls.length === 0) {
            return { output: content, steps: step + 1, agent: agentName };
          }
        ...
        return { output: messages[messages.length - 1]?.content || "Max steps reached", steps: maxSteps, agent: agentName, maxStepsReached: true };
      })();

      return await Promise.race([runPromise, delegateTimeout]);
    } catch (err) {
  ...
  "memory.save": async (args) => {
    const projectDir = args.projectRoot || args.cwd || process.cwd();
    const agentName = args._currentAgent || "default";
  ...
  "memory.load": async (args) => {
    ...
    const filePath = path.join(projectDir, ".agent", "memory", agentName, `${key}.json`);
  ...
  "web.search": async (args) => {
    ...
    let searchConfig = { provider: "serpapi" };
    try {
  ...
  "plan.create": async (args) => {
    const projectDir = args.projectRoot || args.cwd || process.cwd();
    const planName = args.name || `plan-${Date.now()}`;
  ...
  "plan.update": async (args) => {
    ...
    const filePath = path.join(projectDir, ".agent", "plans", `${planId}.json`);
    if (!fs.existsSync(filePath)) {
  ...
  "plan.list": async (args) => {
    ...
    if (args.planId) {
      const filePath = path.join(planDir, `${args.planId}.json`);
