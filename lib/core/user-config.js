...
/**
 * Set default provider, model, and trust level in user config.
 */
function setDefaults(provider, model, trustLevel) {
  const cfg = load();
  if (provider) cfg.default_provider = provider;
  if (model) cfg.default_model = model;
  if (trustLevel) cfg.default_trust_level = trustLevel;
  save(cfg);
  return cfg;
}
...
function display() {
  ...
  lines.push(`  Default trust:    ${cfg.default_trust_level || "auto"}`);
  lines.push("");
...
/**
 * Set default trust level (ask/auto/yolo).
 */
function setTrustLevel(level) {
  if (!["ask", "auto", "yolo"].includes(level)) {
    return { error: `Invalid trust level "${level}". Valid: ask, auto, yolo` };
  }
  const cfg = load();
  cfg.default_trust_level = level;
  save(cfg);
  return { ok: true, trust_level: level };
}
