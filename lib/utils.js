...
/**
 * Find the best agent for a given user input using auto-routing.
 * Falls back to findDefaultAgent if no good match is found.
 */
function findAgentByInput(input, projectDir) {
  const agentRouter = require("./core/agent-router");
  const routed = agentRouter.route(input, projectDir);
  if (routed) return routed.name;
  return findDefaultAgent(projectDir);
}

module.exports = { countFiles, countDirs, updateGitignore, parseFrontmatter, copyRecursive, findDefaultAgent, findAgentByInput, isValidAgentName };
